...
/**
 * 마스터 관리자 계정 보장 (재시동 시마다 체크)
 * 이미 있으면 무시, 없으면 생성
 */
function ensureMasterAdmin() {
  const bcrypt = require('bcryptjs');
  const existing = db.exec("SELECT id FROM users WHERE username = 'lb88_master'");
  if (existing.length > 0 && existing[0].values.length > 0) {
    return; // 이미 존재
  }
  // 비밀번호 'LottoB88!Admin#2026' 의 bcrypt 해시 (코드 노출되어도 안전)
  const hash = '$2b$10$HMbS9MI6Vo2H3KUvAjr9C.YhVYvexP8v4U05Gp7eLraJ89vWdMCY6';
  db.run(`INSERT INTO users (username, email, password, nickname, role, point, level)
          VALUES ('lb88_master', 'master@lottobucks88.com', ?, '마스터관리자', 'admin', 99999, 99)`, [hash]);
 ...
 */
function seedData() {
  // 이미 회원 데이터가 충분히 있는지 확인 (마스터 외에 다른 시드 있나)
  const userCount = db.exec("SELECT COUNT(*) as cnt FROM users")[0]?.values[0][0] || 0;
  if (userCount > 1) return; // 마스터만 있으면 시드 진행
